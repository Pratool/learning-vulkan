...
private:
  ...
  inline std::unique_ptr<VkShaderModule> createShaderModule(const Container& code);
...
template <typename Container>
std::unique_ptr<VkShaderModule> HelloTriangleApplication::createShaderModule(const Container& code)
{
  ...
  return std::make_unique<VkShaderModule>(shaderModule);
}
