...
#include "rendering_and_presentation_fragment_shader/rendering_and_presentation_fragment_shader.hpp"
#include "rendering_and_presentation_vertex_shader/rendering_and_presentation_vertex_shader.hpp"
...
{
  for (std::size_t idx = 0; idx < MAX_FRAMES_IN_FLIGHT; ++idx)
  {
    vkDestroySemaphore(device_, renderFinishedSemaphores_[idx], nullptr);
    vkDestroySemaphore(device_, imageAvailableSemaphores_[idx], nullptr);
    vkDestroyFence(device_, inFlightFences_[idx], nullptr);
  }
...
{
  VkShaderModule vertShaderModule = *(createShaderModule(rendering_and_presentation_vertex_shader()).release());
  VkShaderModule fragShaderModule = *(createShaderModule(rendering_and_presentation_fragment_shader()).release());
...
void HelloTriangleApplication::createFramebuffers()
{
  ...
  for (std::size_t idx = 0; idx < swapChainImageViews_.size(); ++idx)
  {
    VkImageView attachments[] = {swapChainImageViews_[idx]};
    ...
    if (vkCreateFramebuffer(device_, &framebufferInfo, nullptr, &swapChainFramebuffers_[idx]) != VK_SUCCESS)
    {
...
void HelloTriangleApplication::createCommandBuffers()
{
  ...
  for (std::size_t idx = 0; idx < commandBuffers_.size(); ++idx)
  {
    ...
    if (vkBeginCommandBuffer(commandBuffers_[idx], &beginInfo) != VK_SUCCESS)
    {
    ...
    renderPassInfo.framebuffer = swapChainFramebuffers_[idx];
    ...
    vkCmdBeginRenderPass(commandBuffers_[idx], &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);

    vkCmdBindPipeline(commandBuffers_[idx], VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline_);
    vkCmdDraw(commandBuffers_[idx], 3, 1, 0, 0);

    vkCmdEndRenderPass(commandBuffers_[idx]);

    if (vkEndCommandBuffer(commandBuffers_[idx]) != VK_SUCCESS)
    {
...
void HelloTriangleApplication::createSyncObjects()
{
  ...
  for (std::size_t idx = 0; idx < MAX_FRAMES_IN_FLIGHT; ++idx)
  {
    if (vkCreateSemaphore(device_, &semaphoreInfo, nullptr, &imageAvailableSemaphores_[idx]) != VK_SUCCESS
        || vkCreateSemaphore(device_, &semaphoreInfo, nullptr, &renderFinishedSemaphores_[idx]) != VK_SUCCESS
        || vkCreateFence(device_, &fenceInfo, nullptr, &inFlightFences_[idx]) != VK_SUCCESS)
    {
...
void HelloTriangleApplication::drawFrame()
{
  ...
  vkAcquireNextImageKHR(
      device_, swapchain_, UINT64_MAX, imageAvailableSemaphores_[currentFrame_], VK_NULL_HANDLE, &imageIdx);

  // Check if a previous frame is using this image i.e. there is its fence to wait on.
  if (imagesInFlight_[imageIdx] != VK_NULL_HANDLE)
  {
    vkWaitForFences(device_, 1, &imagesInFlight_[imageIdx], VK_TRUE, UINT64_MAX);
  }
  // Mark the image as now being in use by this frame.
  imagesInFlight_[imageIdx] = inFlightFences_[currentFrame_];
  ...
  submitInfo.pCommandBuffers = &commandBuffers_[imageIdx];
